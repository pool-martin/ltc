...
''' 
operate some program or script over all dataset. 
...
def load_args():
	...
	ap.add_argument('-psr', '--program-or-script-to-run',
					dest='program_to_run',
					help='program or script to use over the each entry of the data set (full path).',
					type=str, required=True)
	ap.add_argument('-psa', '--program-or-script-arguments',
					dest='arguments_to_run',
					help='arguments to be passed',
...
def operateOverDataset(video_path, walked_tree, args):

	video_name = os.path.basename(video_path).split('.')[0]

	if "minmax" not in video_path:
		command = args.program_to_run + " " + args.arguments_to_run + " " + video_path
		#print command
...
def iterateOverDataset(folder, cpu_count, args):
	...
	if file_paths:
		...
		Parallel(n_jobs=numjobs, verbose=10)(delayed(operateOverDataset)(file_path, folder[len(args.videos_path)+1:], args) for file_path in file_paths)

	if directory_paths:
		for dir_path in directory_paths:
			iterateOverDataset(dir_path, cpu_count, args)
...
def main():
	...
	cpu_count = 6
	iterateOverDataset(args.videos_path, cpu_count, args)
